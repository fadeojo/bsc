defmodule Bsc.Web.OrgController do
  use Bsc.Web, :controller

  alias Bsc.Medical
  alias Bsc.Medical.Org

  action_fallback Bsc.Web.FallbackController

  def index(conn, _params) do
    orgs = Medical.list_orgs()
    render(conn, "index.json", orgs: orgs)
  end

  def create(conn, %{"org" => org_params}) do
    with {:ok, %Org{} = org} <- Medical.create_org(org_params) do
      conn
      |> put_status(:created)
      |> put_resp_header("location", org_path(conn, :show, org))
      |> render("show.json", org: org)
    end
  end

  def show(conn, %{"id" => id}) do
    org = Medical.get_org!(id)
    render(conn, "show.json", org: org)
  end

  def update(conn, %{"id" => id, "org" => org_params}) do
    org = Medical.get_org!(id)

    with {:ok, %Org{} = org} <- Medical.update_org(org, org_params) do
      render(conn, "show.json", org: org)
    end
  end

  def delete(conn, %{"id" => id}) do
    org = Medical.get_org!(id)
    with {:ok, %Org{}} <- Medical.delete_org(org) do
      send_resp(conn, :no_content, "")
    end
  end
end
